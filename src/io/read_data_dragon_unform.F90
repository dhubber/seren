...
! ============================================================================
SUBROUTINE read_data_dragon_unform(in_file,decomp_read)
  use particle_module
  use hydro_module
  use type_module
  use sink_module
  ...
  integer, allocatable :: auxtype(:)         ! aux. particle type array
...
#if !defined(USE_MPI)
  write(6,*) "Particles   = ", ptot, "   Sinks = ", stot_file
  write(6,*) "Gas         = ", pgas
...
! ----------------------------------------------------------------------------
  ...
  do p=1,(ptot + (stot_file + pdead))
     if (auxtype(p) == 6) then
        minimal_sph(boundaryslot)%r(1:NDIM) = rdummy3(1:NDIM,p)
        minimal_sph(boundaryslot)%porig = p
        boundaryslot = boundaryslot + 1
     end if
     if (auxtype(p) == 9) then
        minimal_sph(icmslot)%r(1:NDIM) = rdummy3(1:NDIM,p)
        minimal_sph(icmslot)%porig = p
        icmslot = icmslot + 1
     end if
     if (auxtype(p) == 1) then
        minimal_sph(gasslot)%r(1:NDIM) = rdummy3(1:NDIM,p)
...
! ----------------------------------------------------------------------------
  ...
  do p=1,(ptot + (stot_file + pdead))
     if (auxtype(p) == 6) then
        minimal_sph(boundaryslot)%v(1:VDIM) = rdummy3(1:VDIM,p)
        boundaryslot = boundaryslot + 1
     end if
     if (auxtype(p) == 9) then
        minimal_sph(icmslot)%v(1:VDIM) = rdummy3(1:VDIM,p)
        icmslot = icmslot + 1
     end if
     if (auxtype(p) == 1) then
        minimal_sph(gasslot)%v(1:VDIM) = rdummy3(1:VDIM,p)
        gasslot = gasslot + 1
     end if
#if defined(SINKS)
     if (auxtype(p) == -1) then
...
! ----------------------------------------------------------------------------
  ...
  do p=1,(ptot + (stot_file + pdead))
     if (auxtype(p) == 6) then
        minimal_sph(boundaryslot)%temp = rdummy1(p)
        boundaryslot = boundaryslot + 1
     end if
     if (auxtype(p) == 9) then
        minimal_sph(icmslot)%temp = rdummy1(p)
        icmslot = icmslot + 1
     end if
     if (auxtype(p) == 1) then
        minimal_sph(gasslot)%temp = rdummy1(p)
        gasslot = gasslot + 1
     end if
  end do
...
! ----------------------------------------------------------------------------
  ...
  do p=1,(ptot + (stot_file + pdead))
     if (auxtype(p) == 6) then
        minimal_sph(boundaryslot)%h = rdummy1(p)
        boundaryslot = boundaryslot + 1
     end if
     if (auxtype(p) == 9) then
        minimal_sph(icmslot)%h = rdummy1(p)
        icmslot = icmslot + 1
     end if
     if (auxtype(p) == 1) then
        minimal_sph(gasslot)%h = rdummy1(p)
        gasslot = gasslot + 1
     end if
#if defined(SINKS)
     if (auxtype(p) == -1) then
...
! ----------------------------------------------------------------------------
  ...
  do p=1,(ptot + (stot_file + pdead))
     if (auxtype(p) == 6) then
        minimal_sph(boundaryslot)%rho = rdummy1(p)
        boundaryslot = boundaryslot + 1
     end if
     if (auxtype(p) == 9) then
        minimal_sph(icmslot)%rho = rdummy1(p)
        icmslot = icmslot + 1
     end if
     if (auxtype(p) == 1) then
        minimal_sph(gasslot)%rho = rdummy1(p)
        gasslot = gasslot + 1
     end if
  end do
...
! ----------------------------------------------------------------------------
  ...
  do p=1,(ptot + (stot_file + pdead))
     if (auxtype(p) == 6) then
        minimal_sph(boundaryslot)%m = rdummy1(p)
        boundaryslot = boundaryslot + 1
     end if
     if (auxtype(p) == 9) then
        minimal_sph(icmslot)%m = rdummy1(p)
        icmslot = icmslot + 1
     end if
     if (auxtype(p) == 1) then
        minimal_sph(gasslot)%m = rdummy1(p)
        gasslot = gasslot + 1
     end if
#if defined(SINKS)
     if (auxtype(p) == -1) then
